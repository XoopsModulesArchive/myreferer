...
function a_myrefererAll_show($options)
{
    ...
    // Referer
    $data = [];
    if (in_array(1, $selected) && Utility::checkRight(1)) {
    ...
    // Engine
    $data = [];
    if (in_array(2, $selected) && Utility::checkRight(2)) {
        $sql = 'SELECT referer, visit, date, ref_url
			FROM ' . $xoopsDB->prefix('myref_referer') . "
    ...
    // Keyword
    $data = [];
    if (in_array(3, $selected) && Utility::checkRight(3)) {
    ...
    // Query
    $data = [];
    if (in_array(4, $selected) && Utility::checkRight(4)) {
        $sql = 'SELECT query, visit, date, page
				FROM ' . $xoopsDB->prefix('myref_query') . "
    ...
    // Robots
    $data = [];
    if (in_array(5, $selected) && Utility::checkRight(5)) {
    ...
    // User
    $data = [];
    if (in_array(6, $selected) && Utility::checkRight(2)) {
