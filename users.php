...
$current = _MYREFERER_USERS;
$xoopsTpl->assign('current', _MYREFERER_USERS);
$xoopsTpl->assign('xoops_pagetitle', _MYREFERER_USERS);
$xoopsTpl->assign('icon', '');

if (Utility::checkRight(6)) {
    // Query : page

    $sql = 'SELECT id, user, visit, startdate, date
    ...
    while (list($id, $user, $visit, $startdate, $date) = $xoopsDB->fetchRow($result)) {
        $info = [];
