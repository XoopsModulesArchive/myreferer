...
if ( myReferer_checkRight(1) ) {
	...
	$count_referer = $xoopsDB -> queryF( "SELECT COUNT(id)
		FROM " . $xoopsDB->prefix("myref_referer")."
		WHERE engine = 0 AND referer != '' $more");

    list( $numrows ) = $xoopsDB->fetchRow($count_referer );
...
//		$info_keywords['referer'] = mb_convert_encoding($query, "", "auto");
	...
	// Query on counter result
	$count_keywords = $xoopsDB -> queryF( "SELECT COUNT(id)
		FROM " . $xoopsDB->prefix("myref_query")."
		WHERE keyword=1 AND query != '' $more");

    list( $numrows ) = $xoopsDB->fetchRow($count_keywords);
...
//		$info_query['referer'] = mb_convert_encoding($query, "", "auto");
	...
	// Query on counter result
	$count_query = $xoopsDB -> queryF( "SELECT COUNT(id)
		FROM " . $xoopsDB->prefix("myref_query")."
		WHERE keyword=0 AND query != '' $more");

    list( $numrows ) = $xoopsDB->fetchRow($count_query);
...
if ( myReferer_checkRight(5) ) {
	...
	// Query on counter result
	$count_robots = $xoopsDB -> queryF( "SELECT COUNT(id)
		FROM " . $xoopsDB->prefix("myref_robots")."
		WHERE robots != '' $more");
	list( $numrows ) = $xoopsDB->fetchRow($count_robots);
...
if ( myReferer_checkRight(6) ) {
	...
	// Query on counter result
	$count_pages = $xoopsDB -> queryF( "SELECT COUNT(id)
		FROM " . $xoopsDB->prefix("myref_pages")."
		WHERE page != '' $more");
	list( $numrows ) = $xoopsDB->fetchRow($count_pages);
...
//		$robot_name = myReferer_GetRobotName($users);
//		$ref_url = myReferer_GetRobotUrl($users);
	...
	// Query on counter result
	$count_users = $xoopsDB -> queryF( "SELECT COUNT(id)
		FROM " . $xoopsDB->prefix("myref_users")."
		WHERE user != '' $more");
	list( $numrows ) = $xoopsDB->fetchRow($count_users);
