...
if ( $op == 'del' AND $id ) {
	if ($confirm) {
		...
		myReferer_adminmenu(1, _MD_MYREFERER_STATS);
		...
		xoops_confirm(array('op' => $op, 'id' => $id, 'confirm' => 1, 'ord' => $ord, 'search' => $search, 'week' => $week, 'start' => $start) , 'stats_visitors.php', _MD_MYREFERER_DELETE_USER . " <br />" . "<br />" . $name . "<br />", _MD_MYREFERER_DELETE);
...
$visitors = "	SELECT * FROM " .
		$xoopsDB->prefix("myref_users") . " f LEFT JOIN " .
		$xoopsDB->prefix("users")  . " p on f.user = p.uid
...
// 			$data_date = formatTimestamp($myrow["date"],'r');
		...
		if ( $myrow["tracker"]==1 ) {
    		$tracker = '&nbsp;<a href="stats_visitors.php?ord='.$ord.'&search='.$search.'&week='.$week.'&op=tracker_off&startart='.$startart.'&id='.$myrow["id"].'">
					<img src="../images/icon/tracker_on.gif" alt="'._MD_MYREFERER_USERVISIT_OFF.'" valign="absmiddle" /></a>';
		} else {
    		$tracker = '&nbsp;<a href="stats_visitors.php?ord='.$ord.'&search='.$search.'&week='.$week.'&op=tracker_on&startart='.$startart.'&id='.$myrow["id"].'">
					<img src="../images/icon/tracker_off.gif" alt="'._MD_MYREFERER_USERVISIT_ON.'" valign="absmiddle" /></a>';
		}
