...
for ($week = 1; $week <= 52; $week++) {
    $query = '   SELECT f.id, f.year, f.week, f.queryid, f.pagesid, f.visit, m.id, m.query, m.visit as visit_now, m.visit_tmp, m.hide FROM ' . $xoopsDB->prefix('myref_query_pages_stats') . ' f LEFT JOIN ' . $xoopsDB->prefix('myref_query') . " m on f.id = m.id
