...
if ('del' === $op and $id) {
    if ($confirm) {
        ...
        xoops_confirm(['op' => $op, 'id' => $id, 'confirm' => 1, 'engine' => $engine, 'ord' => $ord, 'search' => $search, 'week' => $week, 'start' => $start], 'detail_referer.php', _MD_MYREFERER_DELETE_REFERER . ' <br>' . '<br>' . $name . '<br>', _MD_MYREFERER_DELETE);
...
if (0 == $count) {
    ...
    // pages viewed
    $page_query = 'SELECT f.refererid, f.pagesid, p.page, f.visit, f.visit_tmp, f.date, p.hide FROM ' . $xoopsDB->prefix('myref_referer_pages') . ' f LEFT JOIN ' . $xoopsDB->prefix('myref_pages') . ' p on f.pagesid = p.id
