...
if ('del' === $op and $id) {
    if ($confirm) {
        ...
        xoops_confirm(['op'      => $op,
                       ...
                       'week'    => $week,
                       'start'   => $start
                      ], 'detail_keyword.php', _MD_MYREFERER_DELETE_KEYWORD . ' <br>' . '<br>' . $name . '<br>', _MD_MYREFERER_DELETE);
...
$query   = 'SELECT * FROM ' . $xoopsDB->prefix('myreferer_query') . " WHERE id = '$id'";
...
if (0 == $count) {
    ...
    $other_query = 'SELECT id, keyword, query, visit, visit_tmp, hide FROM ' . $xoopsDB->prefix('myreferer_query') . '
  					WHERE id!=' . $myrow['id'] . "
    ...
    $page_query = 'SELECT f.queryid, f.pagesid, m.id, m.page, f.visit, f.visit_tmp, m.hide FROM ' . $xoopsDB->prefix('myreferer_query_pages') . ' f LEFT JOIN ' . $xoopsDB->prefix('myreferer_pages') . ' m on f.pagesid = m.id
    ...
    $all_query = 'SELECT DISTINCT m.id, m.query, m.hide, sum(f.visit_tmp) AS visit_tmp, sum(f.visit) AS visit FROM ' . $xoopsDB->prefix('myreferer_query_pages') . ' f LEFT JOIN ' . $xoopsDB->prefix('myreferer_query') . ' m on f.queryid = m.id
