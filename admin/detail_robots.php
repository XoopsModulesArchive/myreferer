...
if ('del' === $op and $id) {
    if ($confirm) {
        ...
        xoops_confirm(['op'      => $op,
                       ...
                       'week'    => $week,
                       'start'   => $start
                      ], 'detail_robots.php', _MD_MYREFERER_DELETE_ROBOT . ' <br>' . '<br>' . $name . '<br>', _MD_MYREFERER_DELETE);
...
$query   = 'SELECT * FROM ' . $xoopsDB->prefix('myreferer_robots') . " WHERE id = '$id'";
...
if (0 == $count) {
    ...
    $page_query = 'SELECT f.robotsid, f.pagesid, p.page, f.visit, f.visit_tmp, f.date, p.hide FROM ' . $xoopsDB->prefix('myreferer_robots_pages') . ' f LEFT JOIN ' . $xoopsDB->prefix('myreferer_pages') . ' p on f.pagesid = p.id
                  WHERE f.robotsid = ' . $myrow['id'] . ' ORDER BY p.hide, p.page';
