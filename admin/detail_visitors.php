...
if ( $op == 'del' AND $id ) {
	if ($confirm) {
		...
		$sql = "SELECT p.uname FROM " . $xoopsDB->prefix("myref_users") . " f LEFT JOIN " .
				$xoopsDB->prefix("users")  . " p on f.user = p.uid WHERE id=$id";
        $result = $xoopsDB->query( $sql );
        list($name) = $xoopsDB->fetchRow($result);

        xoops_confirm(array('op' => $op, 'id' => $id, 'confirm' => 1, 'ord' => $ord, 'search' => $search, 'week' => $week, 'start' => $start) , 'detail_visitors.php', _MD_MYREFERER_DELETE_ROBOT . " <br />" . "<br />" . $name . "<br />", _MD_MYREFERER_DELETE);
...
$query = "SELECT f.*, p.uname, p.email, p.user_regdate, p.last_login FROM " . $xoopsDB->prefix("myref_users") . " f LEFT JOIN " .
		$xoopsDB->prefix("users")  . " p on f.user = p.uid WHERE id=$id";
...
// Admin
	...
	if ( $myrow["hide"] ) {
       	$status = '&nbsp;<a href="detail_visitors.php?op=d&id='.$myrow["id"].'">
				<img src="../images/icon/on.gif" alt="'._MD_MYREFERER_DISPLAYED.'" /></a>';
	} else {
       	$status = '&nbsp;<a href="detail_visitors.php?op=h&id='.$myrow["id"].'">
				<img src="../images/icon/off.gif" alt="'._MD_MYREFERER_HIDDEN.'" /></a>';
	}

	if ( $myrow["tracker"]==1 ) {
   		$tracker = '&nbsp;<a href="detail_visitors.php?op=tracker_off&id='.$myrow["id"].'">
				<img src="../images/icon/tracker_off.gif" alt="'._MD_MYREFERER_USERVISIT_OFF.'" valign="absmiddle" /></a>';
	} else {
   		$tracker = '&nbsp;<a href="detail_visitors.php?op=tracker_on&id='.$myrow["id"].'">
				<img src="../images/icon/tracker_on.gif" alt="'._MD_MYREFERER_USERVISIT_ON.'" valign="absmiddle" /></a>';
	}
...
// Admin
	...
	// pages viewed
	$page_query = "SELECT f.usersid, f.pagesid, p.page, f.visit, f.visit_tmp, f.date, p.hide FROM " .
    			  $xoopsDB->prefix("myref_users_pages") . " f LEFT JOIN " .
	...
	while ( $page_myrow = $xoopsDB->fetchArray($page_result) )  {
		...
		echo formatTimestamp($page_myrow['date']) .' :&nbsp;<a target="thepage" href="http://' . $page_myrow["page"] . '">' .
