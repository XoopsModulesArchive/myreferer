...
if ('del' === $op and $id) {
    if ($confirm) {
        ...
        xoops_confirm(['op' => $op, 'id' => $id, 'confirm' => 1, 'ord' => $ord, 'search' => $search, 'week' => $week, 'start' => $start], 'detail_pages.php', _MD_MYREFERER_DELETE_PAGE . ' <br>' . '<br>' . $name . '<br>', _MD_MYREFERER_DELETE);
...
if (0 == $count) {
    ...
    $members        = 'SELECT count(r.id) AS nb, sum(p.visit) AS visit, sum(p.visit_tmp) AS visit_tmp, r.startdate, p.date  FROM ' . $xoopsDB->prefix('myref_users_pages') . ' p LEFT JOIN ' . $xoopsDB->prefix('myref_users') . " r on r.id = p.usersid
                    WHERE p.pagesid = '$id'
                    GROUP BY p.pagesid
    ...
    $referer        = 'SELECT count(r.id) AS nb, sum(p.visit) AS visit, sum(p.visit_tmp) AS visit_tmp, r.startdate, p.date  FROM ' . $xoopsDB->prefix('myref_referer_pages') . ' p LEFT JOIN ' . $xoopsDB->prefix('myref_referer') . " r on r.id = p.refererid
    ...
    $search        = 'SELECT count(r.id) AS nb, sum(p.visit) AS visit, sum(p.visit_tmp) AS visit_tmp, r.startdate, p.date  FROM ' . $xoopsDB->prefix('myref_referer_pages') . ' p LEFT JOIN ' . $xoopsDB->prefix('myref_referer') . " r on r.id = p.refererid
    ...
    $keyword        = 'SELECT count(r.id) AS nb, sum(p.visit) AS visit, sum(p.visit_tmp) AS visit_tmp, r.startdate, p.date  FROM ' . $xoopsDB->prefix('myref_query_pages') . ' p LEFT JOIN ' . $xoopsDB->prefix('myref_query') . " r on r.id = p.queryid
    ...
    $query        = 'SELECT count(r.id) AS nb, sum(p.visit) AS visit, sum(p.visit_tmp) AS visit_tmp, r.startdate, p.date  FROM ' . $xoopsDB->prefix('myref_query_pages') . ' p LEFT JOIN ' . $xoopsDB->prefix('myref_query') . " r on r.id = p.queryid
    ...
    $robots        = 'SELECT count(r.id) AS nb, sum(p.visit) AS visit, sum(p.visit_tmp) AS visit_tmp, r.startdate, p.date  FROM ' . $xoopsDB->prefix('myref_robots_pages') . ' p LEFT JOIN ' . $xoopsDB->prefix('myref_robots') . " r on r.id = p.robotsid
                    WHERE p.pagesid = '$id'
                    GROUP BY p.pagesid
