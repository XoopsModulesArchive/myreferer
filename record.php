...
if (is_object($xoopsUser)) {
	...
	if ( $count_admin ) { $record = 0; }
}


//  ------------------------------------------------------------------------------------------------------------- //
// 0) Restoring visit_tmp by 0
 if ( date('w') == 1 ) { myreferer_restoring(); }
...
$hide = '0';
if ( $module_blacklist && preg_match('/(' . $module_blacklist . ')/i', $page) ) { $hide = '1'; }
//  ------------------------------------------------------------------------------------------------------------- //
// Check page
...
if ( (eregi('Mozilla', $robots )  OR eregi('Opera', $robots )) AND !eregi('googlebot', $robots) AND is_object($xoopsUser)) {
...
if ( !eregi(XOOPS_URL, $ref_url ) AND $ref_url ) {
...
//	}
	...
	$variables['p'] 	= isset($variables['p']) 	? $variables['p'] 	: "";
	$variables['q'] 	= isset($variables['q']) 	? $variables['q'] 	: "";
	$variables['qs'] 	= isset($variables['qs']) 	? $variables['qs'] 	: "";
	$variables['as_q'] 	= isset($variables['as_q']) 	? $variables['as_q'] 	: "";
	$variables['q1'] 	= isset($variables['q1']) 	? $variables['q1'] 	: "";
	$variables['word'] 	= isset($variables['word']) 	? $variables['word'] 	: "";
	$variables['query'] 	= isset($variables['query']) 	? $variables['query'] 	: "";
	$variables['keywords'] 	= isset($variables['keywords']) ? $variables['keywords'] : "";
	$variables['KeyWords'] 	= isset($variables['KeyWords']) ? $variables['KeyWords'] : "";
	$variables['Keywords'] 	= isset($variables['Keywords']) ? $variables['Keywords'] : "";
	$variables['strKeywords'] = isset($variables['strKeywords']) ? $variables['strKeywords'] : "";
        $variables['search'] 	= isset($variables['search'])   ? $variables['search'] : "";
	$variables['Search'] 	= isset($variables['Search'])   ? $variables['Search'] : "";
	$variables['searchfor'] = isset($variables['searchfor']) ? $variables['searchfor'] : "";
	...
	if ((  $variables['p']
		...
		OR $variables['query']
		OR $variables['word']
		OR $variables['keywords']
		OR $variables['KeyWords']
		OR $variables['strKeywords']
		OR $variables['search']
		OR $variables['Search']
		OR $variables['searchfor']
	    ) AND !eregi('=cache', $url_array['query']) ) {
		...
		$keywords = myreferer_clean_queries( $query, $keyword_min, $keyword_max, $punctation, $numbers, $smallcaps, 1);
        ...
        if ( is_array($keywords) && count($keywords) > 1 ) {
			...
			$query = myreferer_clean_queries( $query, $keyword_min, $keyword_max, $punctation, $numbers, $smallcaps, 0 );
		...
		if( eregi('=cache', $url_array['query']) ) { $engine = 1; } else { $engine = 0; }
	}
...
if ( !eregi('Mozilla', $robots )  AND !eregi('Opera', $robots ) OR eregi('googlebot', $robots) ) {
	...
	$robot_name = myReferer_GetRobotName($robots);
	$ref_url = myReferer_GetRobotUrl($robots);
	...
	if ( $xoopsDB->getRowsNum($result) == 0 ) {
        ...
        // send for new bot
        if ( $new_bot_smail ) {
        	myreferer_sendmail ( 1 , $new_bot_mail, $robot_name, $ref_url, $page ) ;
        }
...
//		if ( $tracker ) {
//        	myreferer_sendmail ( 0 , $new_bot_mail, $robot_name, $ref_url, $page ) ;
//		}
	}
