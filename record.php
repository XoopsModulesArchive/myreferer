...
$sql_page    = 'SELECT id FROM ' . $xoopsDB->prefix('myreferer_pages') . " WHERE page='$page'";
...
if (!$pagesid) {
    ...
    if ($myrefererIsAdmin && !$count_admin) {
        $sql = 'INSERT INTO ' . $xoopsDB->prefix('myreferer_pages') . "
    ...
    if ($count_admin) {
        $sql = 'UPDATE ' . $xoopsDB->prefix('myreferer_pages') . "
...
if (is_object($xoopsUser)) {
    ...
    foreach ($groups as $key => $group) {
        if (in_array($group, $uid_groups, true)) {
            $save_gpage = 1;
...
if ((preg_match('Mozilla', $robots) || preg_match('Opera', $robots)) && false === mb_strpos($robots, 'oo') && is_object($xoopsUser)) {
    ...
    if (0 == $xoopsDB->getRowsNum($result)) {
        ...
        $sql = 'INSERT INTO ' . $xoopsDB->prefix('myreferer_users') . "
        ...
        if ($diff >= 3600) {
            $sql = 'UPDATE ' . $xoopsDB->prefix('myreferer_users') . "
				SET visit = visit+1, visit_tmp = visit_tmp+1, date = '$date'
				WHERE user = $uid";
        } else {
            $sql = 'UPDATE ' . $xoopsDB->prefix('myreferer_users') . "
...
if ($user_visit and is_object($xoopsUser)) {
    if ($save_gpage && $save_upage) {
        ...
        $sql = 'SELECT id FROM ' . $xoopsDB->prefix('myreferer_users_pages') . " WHERE usersid='$userid' AND pagesid='$pagesid'";

        $result = $xoopsDB->query($sql);

        if (0 == $xoopsDB->getRowsNum($result)) {
            $sql = 'INSERT INTO ' . $xoopsDB->prefix('myreferer_users_pages') . "
					VALUES ('', '$userid', '$pagesid', '1', '1', '$date', '$date', '$hide', '0')";

            $xoopsDB->queryF($sql);
        } else {
            $sql = 'UPDATE ' . $xoopsDB->prefix('myreferer_users_pages') . "
...
if (!preg_match(XOOPS_URL, $ref_url) and $ref_url) {
        ...
        if (is_array($keywords)) {
            foreach ($keywords as $keyword) {
                if ('' != $keyword) {
                    ...
                    if (0 == $xoopsDB->getRowsNum($result)) {
                        $sql = 'INSERT INTO ' . $xoopsDB->prefix('myreferer_query') . "
                        ...
                        $sql = 'UPDATE ' . $xoopsDB->prefix('myreferer_query') . "
                    ...
                    $sql = 'SELECT id FROM ' . $xoopsDB->prefix('myreferer_query_pages') . " WHERE queryid='$queryid' AND pagesid='$pagesid'";

                    $result = $xoopsDB->query($sql);

                    if (0 == $xoopsDB->getRowsNum($result)) {
                        $sql = 'INSERT INTO ' . $xoopsDB->prefix('myreferer_query_pages') . "
							VALUES ('', '$queryid', '$pagesid', '1', '1', '$date', '$date', '$hide', '0')";

                        $xoopsDB->queryF($sql);
                    } else {
                        $sql = 'UPDATE ' . $xoopsDB->prefix('myreferer_query_pages') . "
						SET visit = visit+1, visit_tmp = visit_tmp+1, date = '$date', tracker='0'
    ...
    $sql = 'SELECT id FROM ' . $xoopsDB->prefix('myreferer_referer') . "	WHERE referer = '$referer' ";
    ...
    if (0 == $xoopsDB->getRowsNum($result)) {
        $sql = 'INSERT INTO ' . $xoopsDB->prefix('myreferer_referer') . "
        ...
        $sql = 'UPDATE ' . $xoopsDB->prefix('myreferer_referer') . "
    ...
    $sql = 'SELECT id FROM ' . $xoopsDB->prefix('myreferer_referer_pages') . " WHERE refererid='$refererid' AND pagesid='$pagesid'";

    $result = $xoopsDB->query($sql);

    if (0 == $xoopsDB->getRowsNum($result)) {
        $sql = 'INSERT INTO ' . $xoopsDB->prefix('myreferer_referer_pages') . "
				VALUES ('', '$refererid', '$pagesid', '1', '1', '$date', '$date', '$hide', '0')";

        $xoopsDB->queryF($sql);
    } else {
        $sql = 'UPDATE ' . $xoopsDB->prefix('myreferer_referer_pages') . "
			SET visit = visit+1, visit_tmp = visit_tmp+1, date = '$date', tracker='0'
...
if (!preg_match('Mozilla', $robots) && !preg_match('Opera', $robots) || false !== mb_strpos($robots, 'oo')) {
    ...
    $sql = 'SELECT id, tracker FROM ' . $xoopsDB->prefix('myreferer_robots') . " WHERE robots = '$robots' ";
    ...
    if (0 == $xoopsDB->getRowsNum($result)) {
        $sql = 'INSERT INTO ' . $xoopsDB->prefix('myreferer_robots') . "
        ...
        $sql = 'UPDATE ' . $xoopsDB->prefix('myreferer_robots') . "
    ...
    $sql = 'SELECT id, tracker FROM ' . $xoopsDB->prefix('myreferer_robots_pages') . " WHERE robotsid='$robotsid' AND pagesid='$pagesid'";

    $result = $xoopsDB->query($sql);

    if (0 == $xoopsDB->getRowsNum($result)) {
        $sql = 'INSERT INTO ' . $xoopsDB->prefix('myreferer_robots_pages') . "
        ...
        $sql = 'UPDATE ' . $xoopsDB->prefix('myreferer_robots_pages') . "
			SET visit = visit+1, visit_tmp = visit_tmp+1, date = '$date', tracker='0'
