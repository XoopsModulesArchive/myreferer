...
if ( (myReferer_checkRight(3) && $op==1) || (myReferer_checkRight(4) && $op==0) ) {
	$form_letter = '';
	$first_letter = '';
	...
	while(list( $id, $query, $page, $visit, $startdate, $date) = $xoopsDB->fetchRow($result)) {
		...
		if ( $ord == 'alpha' ) {
			...
			if ( $letter != $form_letter ) { $first_letter = '<h3>'.$letter.'</h3>'; $visit = '['.$visit.'] ';} else { $first_letter = ''; $visit = '<br />['.$visit.'] ';}
			$form_letter = $letter;
		} else {
        	$count++;
        }

        if ( $page ) { $page = 'http://'.$page; }
