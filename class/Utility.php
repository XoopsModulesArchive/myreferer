...
';
    ...
    public static function getMeta($key)
    {
        ...
        $sql = \sprintf('SELECT conf_value FROM %s WHERE conf_name=%s', $xoopsDB->prefix('myreferer_config'), $xoopsDB->quoteString($key));
    ...
    public static function setMeta($key, $value)
    {
        ...
        if (self::getMeta($key)) {
            $sql = \sprintf('UPDATE %s SET conf_value = %s WHERE conf_name = %s', $xoopsDB->prefix('myreferer_config'), $xoopsDB->quoteString($value), $xoopsDB->quoteString($key));
        } else {
            $sql = \sprintf("INSERT INTO %s (conf_id , conf_name, conf_title, conf_value, conf_desc, conf_formtype, conf_valuetype, conf_order) 
        VALUES (0, %s, '', %s, '', 'hidden', 'hidden', 0)", $xoopsDB->prefix('myreferer_config'), $xoopsDB->quoteString($key), $xoopsDB->quoteString($value));
        }
    ...
    public static function displayUrl($url, $length = '20')
    {
        ...
        if ('' != $urls[2] && mb_strlen($urls[2]) > $length) {
            $urls = \explode('?', $urls[2]);
