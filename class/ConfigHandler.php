...
class ConfigHandler extends XoopsObjectHandler
{
    ...
    {
        if ((int)$id > 0) {
            $sql = 'SELECT * FROM ' . $this->db->prefix('myreferer_config') . ' WHERE conf_id=' . $id;
    ...
    public function insert(XoopsObject $config)
    {
        ...
        if ($config->isNew()) {
            ...
            $sql = \sprintf('INSERT INTO %s (conf_id, conf_name, conf_title, conf_value, conf_desc, conf_formtype, conf_valuetype, conf_order) VALUES (%u, %u, %u, %s, %s, %s, %s, %s, %s, %u)', $this->db->prefix('myreferer_config'), $conf_id,
                            $this->db->quoteString($conf_name), $this->db->quoteString($conf_title), $this->db->quoteString($conf_value), $this->db->quoteString($conf_desc), $this->db->quoteString($conf_formtype),
                            $this->db->quoteString($conf_valuetype), $conf_order);
        } else {
            $sql = \sprintf('UPDATE %s SET conf_name = %s, conf_title = %s, conf_value = %s, conf_desc = %s, conf_formtype = %s, conf_valuetype = %s, conf_order = %u WHERE conf_id = %u', $this->db->prefix('myreferer_config'),
                            $this->db->quoteString($conf_name), $this->db->quoteString($conf_title), $this->db->quoteString($conf_value), $this->db->quoteString($conf_desc), $this->db->quoteString($conf_formtype),
    ...
    public function getAll($criteria = null, $sort = 'conf_order', $order = 'ASC', $asobject = true)
    {
        ...
        $sql = 'SELECT * FROM ' . $this->db->prefix('myreferer_config');
